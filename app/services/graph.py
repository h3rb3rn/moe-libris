...
async def commit_bundle(bundle_data: dict, origin_node_id: str) -> dict:
    ...
    Raises ValueError if the Neo4j commit fails.
    """
    try:
        driver = await get_driver()
        ...
        async with driver.session() as session:
            ...
            for rel in bundle_data.get("relations", []):
                ...
                if predicate not in VALID_PREDICATES:
                    logger.warning(
                        "Skipping relation with invalid predicate %r (not in allowlist)",
                        predicate,
                    )
                    continue
        ...
        logger.error("Neo4j commit_bundle failed: %s", e)
        raise ValueError(f"Failed to commit bundle to Neo4j: {e}") from e
...
async def pull_since(
    ...
    Raises ValueError if the Neo4j query fails.
    """
    try:
        driver = await get_driver()
        entities = []
        ...
        logger.error("Neo4j pull_since failed: %s", e)
        raise ValueError(f"Failed to pull from Neo4j: {e}") from e
